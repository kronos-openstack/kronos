...
| **M7** | Replay reuses the engine: dependency-inject Nova/Prometheus clients and the cooldown tracker into `EngineLoop`, expose a public `run_once()`, and shrink `kronos-replay` to a thin wrapper. Snapshot format gets a host->zone map so the AZ filter still applies; `--time` becomes an opt-in timing hook on the engine itself. | Planned |
| **M8** | Audit logging (append-only JSONL) and general logging cleanup: no leading whitespace, no multiline LOG calls, single format string per call (OpenStack-standard oslo.log style) | Planned |
