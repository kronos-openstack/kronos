...
class TestPolicyConfig:
    ...
    def test_invalid_name_uppercase(self):
        with pytest.raises(ValidationError, match="name"):
            PolicyConfig.model_validate(
                {"name": "BadName", "mode": "spread",
                 "weight": 1.0, "imbalance_query": "up"},
            )

    def test_invalid_name_starts_with_number(self):
        with pytest.raises(ValidationError, match="name"):
            PolicyConfig.model_validate(
                {"name": "1-bad", "mode": "spread",
                 "weight": 1.0, "imbalance_query": "up"},
            )

    def test_valid_name_with_hyphens_underscores(self):
        policy = PolicyConfig.model_validate(
            {"name": "my-cool_policy-1", "mode": "spread",
             "weight": 1.0, "imbalance_query": "up"},
        )
    ...
    def test_threshold_range(self):
        with pytest.raises(ValidationError, match="threshold"):
            PolicyConfig.model_validate(
                {"name": "bad-threshold", "mode": "spread",
                 "weight": 1.0, "imbalance_query": "up", "threshold": 1.5},
            )

    def test_weight_must_be_zero_or_positive(self):
        policy = PolicyConfig.model_validate(
            {"name": "zero-weight", "mode": "spread",
             "weight": 0.0, "imbalance_query": "up"},
        )
    ...
    def test_weight_must_not_exceed_one(self):
        with pytest.raises(ValidationError, match="weight"):
            PolicyConfig.model_validate(
                {"name": "huge-weight", "mode": "spread",
                 "weight": 1.5, "imbalance_query": "up"},
            )
...
class TestPoliciesConfig:
    ...
    def test_valid_two_spread_policies_summing_to_one(self, sample_policy_dict):
        a = {**sample_policy_dict, "name": "alpha", "weight": 0.4}
        b = {**sample_policy_dict, "name": "beta", "weight": 0.6}
    ...
    def test_weights_not_summing_to_one_rejected(self, sample_policy_dict):
        a = {**sample_policy_dict, "name": "alpha", "weight": 0.3}
        b = {**sample_policy_dict, "name": "beta", "weight": 0.3}
    ...
    def test_disabled_policy_weight_excluded_from_sum(self, sample_policy_dict):
        a = {**sample_policy_dict, "name": "alpha", "weight": 1.0}
        b = {**sample_policy_dict, "name": "beta", "weight": 0.7, "enabled": False}
        config = PoliciesConfig(
            policies=[PolicyConfig(**a), PolicyConfig(**b)],
        )
        assert len(config.policies) == 2
