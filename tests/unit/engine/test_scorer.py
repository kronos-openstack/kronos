...
@pytest.fixture()
def scorer(mock_prometheus):
    return PolicyScorer(mock_prometheus)


class TestPolicyScorerEvaluate:
    def test_balanced_hosts(self, scorer, mock_prometheus):
        mock_prometheus.instant_query.return_value = _make_query_result(
            {"h1": 0.50, "h2": 0.52, "h3": 0.48}
        )

        policy = _make_policy(threshold=0.15)
        result = scorer.evaluate(policy, ["h1", "h2", "h3"])
    ...
    def test_imbalanced_hosts(self, scorer, mock_prometheus):
        mock_prometheus.instant_query.return_value = _make_query_result(
    ...
    def test_empty_hosts(self, scorer, mock_prometheus):
        result = scorer.evaluate(_make_policy(), [])

        assert result.skipped
        assert "no hosts" in result.skip_reason.lower()

    def test_untrustworthy_data_skipped(self, scorer, mock_prometheus):
        mock_prometheus.instant_query.return_value = _make_query_result(
            {"h1": 0.5},
        ...
        result = scorer.evaluate(_make_policy(), ["h1", "h2"])

        assert result.skipped
        assert "untrustworthy" in result.skip_reason.lower()

    def test_no_matching_hosts_in_prometheus(self, scorer, mock_prometheus):
        mock_prometheus.instant_query.return_value = _make_query_result(
            {"other-host": 0.5}
        )
    ...
    def test_out_of_range_values_skipped(self, scorer, mock_prometheus):
        mock_prometheus.instant_query.return_value = _make_query_result(
            {"h1": 0.5, "h2": 1.7},  # 1.7 is out of [0, 1]
        )

        result = scorer.evaluate(_make_policy(), ["h1", "h2"])

        assert result.skipped
        assert "out-of-range" in result.skip_reason.lower()

    def test_evaluation_duration_tracked(self, scorer, mock_prometheus):
        mock_prometheus.instant_query.return_value = _make_query_result({"h1": 0.5})

        result = scorer.evaluate(_make_policy(), ["h1"])
        assert result.evaluation_duration_ms >= 0

    def test_scores_sorted_descending(self, scorer, mock_prometheus):
        mock_prometheus.instant_query.return_value = _make_query_result(
            {"h1": 0.30, "h2": 0.80, "h3": 0.50}
        )

        result = scorer.evaluate(_make_policy(), ["h1", "h2", "h3"])
        raw_scores = [hs.raw_score for hs in result.host_scores]
        assert raw_scores == sorted(raw_scores, reverse=True)

    def test_filters_non_scope_hosts(self, scorer, mock_prometheus):
        mock_prometheus.instant_query.return_value = _make_query_result(
...
class TestNormalizeScores:
    ...
    def test_single_host(self):
        ...
        scores = PolicyScorer._normalize_scores(raw)
        assert len(scores) == 1
        assert scores[0].raw_score == 0.42
